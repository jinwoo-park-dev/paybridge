document.addEventListener('DOMContentLoaded', async () => {
  ...
  if (!publishableKey || !clientSecret || !returnUrl) {
    setMessage('Stripe page is missing configuration. Check local env vars and server-created PaymentIntent state.', true);
    return;
  }
  ...
  form.addEventListener('submit', async (event) => {
    ...
    try {
      ...
      setMessage(error && error.message ? error.message : 'Unexpected Stripe confirmation error. Please try a fresh demo order ID.', true);
      restoreButton();
    }
  });
});
